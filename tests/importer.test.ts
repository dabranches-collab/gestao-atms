...
function workbook(rows: unknown[][]) { const book=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(book,XLSX.utils.aoa_to_sheet([headers,...rows]),'CA-BCI-202607'); XLSX.utils.book_append_sheet(book,XLSX.utils.aoa_to_sheet([['Nome do Report','LISTAGEM CA'],['Data da Informação','202607'],['Banco/Entidade','5'],['Classificação','Reservada']]),'INFO_REPORT'); return XLSX.write(book,{type:'array',bookType:'xlsx'}) as ArrayBuffer }
describe('importação Excel',()=>{
  ...
  it('ignora o sufixo de cópia criado pelo Windows',()=>expect(parseReportFilename('L00046010-202607-LISTAGEM_CA_BAT-BCI (1).xlsx').client).toBe('BCI'))
  ...
  it('aceita o código de entidade real do BCI',()=>expect(parseWorkbook(workbook([['001','BCI',10,200,5,1,500,400,80,.1]]),'L00046010-202607-LISTAGEM_CA_BAT-BCI (1).xlsx').issues.some(i=>i.code==='METADATA_CLIENT_MISMATCH')).toBe(false))
})
